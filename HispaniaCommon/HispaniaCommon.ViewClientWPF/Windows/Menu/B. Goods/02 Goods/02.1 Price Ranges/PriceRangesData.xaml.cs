...
{
    /// <summary>
    /// Interaction logic for PriceRanges.xaml
    /// </summary>
    public partial class PriceRangesData : UserControl
    {
        #region Constants
        ...
        /// </summary>
        /// <param name="NewOrEditedPriceRangesView">New or Edited PriceRange.</param>
        public delegate void dlgAccept(PriceRangesView NewOrEditedPriceRangesView);
        ...
        /// </summary>
        private PriceRangesView m_PriceRanges = null;
        ...
        /// </summary>
        public PriceRangesView PriceRangeData
        {
            get
            {
            ...
            set
            {
                if (value != null)
                {
                    ...
                    m_PriceRanges = value;
                    EditedPriceRanges = new PriceRangesView(m_PriceRanges);
                    LoadDataInControls(m_PriceRanges);
                }
                else throw new ArgumentNullException("Error, no s'han trobat les dades de la relació de Preus de Venta al public aplicables a carregar."); 
            }
        }
        ...
        public Operation CtrlOperation
        {
            ...
            set
            {
                ...
                {
                    case Operation.Show :
                         if (PriceRangeData == null) throw new InvalidOperationException("Error, impossible visualitzar una relació de Preus de Venta al Public aplicables sense dades.");
                         tbCancel.Text = "Tornar";
                         break;
                    case Operation.Add:
                         PriceRangeData = new PriceRangesView(Data.Good_Id);
                         tbCancel.Text = "Cancel·lar";
                         break;
                    case Operation.Edit:
                         if (PriceRangeData == null) throw new InvalidOperationException("Error, impossible editar una la relació de Preus de Venta al Public aplicables sense dades.");
                         tbCancel.Text = "Cancel·lar";
                         break;
                }
        ...
        /// </summary>
        private PriceRangesView EditedPriceRanges
        {
            get;
            set;
        }
        ...
        private void InitEditableControls()
        {
            ...
            {
                lblPriceRangeSequence,
                tbPriceRangeSequence,
                lblSince,
                tbSince,
                lblUntil,
                tbUntil
        ...
        /// </summary>
        private void LoadDataInControls(PriceRangesView priceRangesView)
        {
            DataChangedManagerActive = false;
            tbGood.Text = Data.Good_Code;
            tbGoodDescription.Text = Data.Good_Description;
            tbPriceRangeSequence.Text = GlobalViewModel.GetStringFromDecimalValue(priceRangesView.Sequence, DecimalType.WithoutDecimals);
            tbSince.Text = GlobalViewModel.GetStringFromDecimalValue(priceRangesView.Since, DecimalType.Currency);
            tbUntil.Text = GlobalViewModel.GetStringFromDecimalValue(priceRangesView.Until, DecimalType.Currency);
        ...
        private void LoadManagers()
        {
            //  By default the manager for the Customer Data changes is active.
                DataChangedManagerActive = true;
            //  TextBox
                tbPriceRangeSequence.PreviewTextInput += TBPreviewTextInput;
                tbPriceRangeSequence.TextChanged += TBDataChanged;
                tbSince.PreviewTextInput += TBPreviewTextInput;
                tbSince.TextChanged += TBCurrencyDataChanged;
                tbUntil.PreviewTextInput += TBPreviewTextInput;
                tbUntil.TextChanged += TBCurrencyDataChanged;
            //  Buttons
        ...
        {
            if (sender == tbPriceRangeSequence) e.Handled = !GlobalViewModel.IsValidShortDecimalChar(e.Text, true);
            else if ((sender == tbSince) || (sender == tbUntil))
            {
        ...
        private void TBDataChanged(object sender, TextChangedEventArgs e)
        {
            if (DataChangedManagerActive)
            {
                ...
                try
                {
                    GlobalViewModel.NormalizeTextBox(sender, e, DecimalType.Normal);
                    decimal value = GlobalViewModel.GetUIDecimalValue(tbInput.Text);
                    if (sender == tbPriceRangeSequence) EditedPriceRanges.Sequence = value;
                }
                catch (Exception ex)
                {
                    MsgManager.ShowMessage(MsgManager.ExcepMsg(ex));
                    LoadDataInControls(EditedPriceRanges);
                }
                AreDataChanged = (EditedPriceRanges != PriceRangeData);
                DataChangedManagerActive = true;
            }
        }
        ...
        private void TBCurrencyDataChanged(object sender, TextChangedEventArgs e)
        {
            if (DataChangedManagerActive)
            {
                ...
                try
                {
                    ...
                    decimal value = GlobalViewModel.GetUIDecimalValue(tbInput.Text);
                    if (sender == tbSince) EditedPriceRanges.Since = value;
                    else if (sender == tbUntil) EditedPriceRanges.Until = value;
                }
                catch (Exception ex)
                {
                    MsgManager.ShowMessage(MsgManager.ExcepMsg(ex));
        ...
        {
            PriceRangesAttributes ErrorField = PriceRangesAttributes.None;
            try
            {
                if ((CtrlOperation == Operation.Add) || (CtrlOperation == Operation.Edit))
                {
                    EditedPriceRanges.Validate(out ErrorField);
                    EvAccept?.Invoke(new PriceRangesView(EditedPriceRanges));
                }
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Restore all source values.
        /// </summary>
        public void RestoreSourceValues()
        {
            EditedPriceRanges.RestoreSourceValues(PriceRangeData);
            LoadDataInControls(EditedPriceRanges);
            AreDataChanged = (EditedPriceRanges != PriceRangeData);
        }
        ...
        public void RestoreSourceValue(PriceRangesAttributes ErrorField)
        {
            EditedPriceRanges.RestoreSourceValue(PriceRangeData, ErrorField);
            LoadDataInControls(EditedPriceRanges);
            AreDataChanged = (EditedPriceRanges != PriceRangeData);
        }
