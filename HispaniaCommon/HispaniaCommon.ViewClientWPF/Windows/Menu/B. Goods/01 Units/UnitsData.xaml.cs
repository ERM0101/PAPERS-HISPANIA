...
namespace HispaniaCommon.ViewClientWPF.UserControls
{
    ...
    /// </summary>
    public partial class UnitsData : UserControl
    {
        #region Constants
        ...
        /// </summary>
        /// <param name="NewOrEditedUnitsView">New or Edited Unit.</param>
        public delegate void dlgAccept(UnitsView NewOrEditedUnitsView);
        ...
        /// </summary>
        private UnitsView m_Units = null;
        ...
        public UnitsView UnitData
        {
            ...
            set
            {
                if (value != null)
                {
                    ...
                    m_Units = value;
                    EditedUnits = new UnitsView(m_Units);
                    LoadDataInControls(m_Units);
                }
                else throw new ArgumentNullException("Error, no s'han trobat les dades de la Unitat a carregar."); 
            }
        }
        ...
        public Operation CtrlOperation
        {
            ...
            set
            {
                ...
                {
                    case Operation.Show :
                         if (UnitData == null) throw new InvalidOperationException("Error, impossible visualitzar una Unitat sense dades.");
                         tbCancel.Text = "Tornar";
                         break;
                    case Operation.Add:
                         UnitData = new UnitsView();
                         tbCancel.Text = "Cancel·lar";
                         break;
                    case Operation.Edit:
                         if (UnitData == null) throw new InvalidOperationException("Error, impossible editar una Unitat sense dades.");
                         tbCancel.Text = "Cancel·lar";
                         break;
                }
        ...
        /// </summary>
        private void LoadDataInControls(UnitsView UnitsView)
        {
            DataChangedManagerActive = false;
            tbUnitsCode.Text = UnitsView.Code;
            tbShipping.Text = UnitsView.Shipping;
            tbBilling.Text = UnitsView.Billing;
        ...
        private void LoadManagers()
        {
            //  By default the manager for the Customer Data changes is active.
                DataChangedManagerActive = true;
            //  TextBox
                tbUnitsCode.PreviewTextInput += TBPreviewTextInput;
                tbUnitsCode.TextChanged += TBDataChanged;
                tbShipping.PreviewTextInput += TBPreviewTextInput;
                tbShipping.TextChanged += TBDataChanged;
                tbBilling.PreviewTextInput += TBPreviewTextInput;
                tbBilling.TextChanged += TBDataChanged;
            //  Buttons
        ...
        {
            if (sender == tbUnitsCode) e.Handled = !GlobalViewModel.IsValidUnitCodeChar(e.Text);
            else if ((sender == tbShipping) || (sender == tbBilling))
            {
        ...
        private void TBDataChanged(object sender, TextChangedEventArgs e)
        {
            if (DataChangedManagerActive)
            {
                ...
                try
                {
                    if (sender == tbUnitsCode) EditedUnits.Code = value;
                    else if (sender == tbShipping) EditedUnits.Shipping = value;
                    else if (sender == tbBilling) EditedUnits.Billing = value;
                }
                catch (Exception ex)
                {
                    MsgManager.ShowMessage(MsgManager.ExcepMsg(ex));
                    LoadDataInControls(EditedUnits);
                }
                AreDataChanged = (EditedUnits != UnitData);
                DataChangedManagerActive = true;
            }
        }
        ...
        {
            UnitsAttributes ErrorField = UnitsAttributes.None;
            try
            {
                if ((CtrlOperation == Operation.Add) || (CtrlOperation == Operation.Edit))
                {
                    EditedUnits.Validate(out ErrorField);
                    EvAccept?.Invoke(new UnitsView(EditedUnits));
                }
            }
            catch (Exception ex)
            {
        ...
        {
            EditedUnits.RestoreSourceValues(UnitData);
            LoadDataInControls(EditedUnits);
            AreDataChanged = (EditedUnits != UnitData);
        }
        ...
        public void RestoreSourceValue(UnitsAttributes ErrorField)
        {
            EditedUnits.RestoreSourceValue(UnitData, ErrorField);
            LoadDataInControls(EditedUnits);
            AreDataChanged = (EditedUnits != UnitData);
        }
