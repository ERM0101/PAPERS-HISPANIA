...
namespace HispaniaCommon.ViewClientWPF.Windows
{
    ...
    public partial class Units : Window
    {
        ...
        /// </summary>
        private ObservableCollection<UnitsView> m_DataList = new ObservableCollection<UnitsView>();
        ...
        /// Store the data to show in List of Items.
        /// </summary>
        public ObservableCollection<UnitsView> DataList
        {
            get
            {
                return (m_DataList);
            ...
            set
            {
                if (value != null)
                {
                    ...
                    CollectionViewSource.GetDefaultView(ListItems.ItemsSource).SortDescriptions.Add(new SortDescription("Code_Value", ListSortDirection.Ascending));
                }
                else m_DataList = new ObservableCollection<UnitsView>();
        ...
        public Units(ApplicationType AppType)
        {
            InitializeComponent();
            Initialize(AppType);
        }
        ...
        private void ActualizeWindowComponents(ApplicationType AppType)
        {
            ...
            //  Apply Theme to window.
                ...
                UnitsDataControl.AppType = AppType;
        ...
        private void LoadDataInWindowComponents()
        {
            ...
            //  Set Data into the Window.
                ...
                CollectionViewSource.GetDefaultView(ListItems.ItemsSource).Filter = UserFilter;
                cbFieldItemToSearch.ItemsSource = UnitsView.Fields;
                cbFieldItemToSearch.DisplayMemberPath = "Key";
                cbFieldItemToSearch.SelectedValuePath = "Value";
                if (UnitsView.Fields.Count > 0) cbFieldItemToSearch.SelectedIndex = 0;
            //  Deactivate managers
        ...
        private bool UserFilter(object item)
        {
            ...
            //  Get Acces to the object and the property name To Filter.
                UnitsView ItemData = (UnitsView)item;
        ...
        private void LoadManagers()
        {
            ...
            //  Define CustomerDataControl events to manage.
                UnitsDataControl.EvAccept += UnitsDataControl_evAccept;
                UnitsDataControl.EvCancel += UnitsDataControl_evCancel;
        }
        ...
        private void BtnRefresh_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ActualizeUnitsFromDb();
            }
            catch (Exception ex)
            {
                MsgManager.ShowMessage(string.Format("Error, al refrescar els valors de les Unitats.", MsgManager.ExcepMsg(ex)));
            }
        }

        #endregion

        #region Editing Units
        ...
        private void BtnViewData_Click(object sender, RoutedEventArgs e)
        {
            if (ListItems.SelectedItem != null)
            {
                ...
                {
                    ActualizeUnitFromDb();
                    UnitsDataControl.CtrlOperation = Operation.Show;
                    gbEditOrCreateItem.SetResourceReference(Control.StyleProperty, "NonEditableGroupBox");
                    ShowItemPannel();
                }
                catch (Exception ex)
                {
                    MsgManager.ShowMessage(string.Format("Error, a l'intentar veure les dades de la Unitat seleccionada.\r\nDetalls: {0}", MsgManager.ExcepMsg(ex)));
                }
            }
        }
        ...
        {
            UnitsDataControl.CtrlOperation = Operation.Add;
        ...
        private void BtnEdit_Click(object sender, RoutedEventArgs e)
        {
            if (ListItems.SelectedItem != null)
            {
                try
                {
                    ...
                    {
                        UnitsDataControl.CtrlOperation = Operation.Edit;
                        gbEditOrCreateItem.SetResourceReference(Control.StyleProperty, "EditableGroupBox");
                        ShowItemPannel();
                    }
                ...
                {
                    MsgManager.ShowMessage(string.Format("Error, a l'iniciar l'edició de la Unitat seleccionada.\r\nDetalls: {0}", MsgManager.ExcepMsg(ex)));
                }
            }
        }
        ...
        private void BtnDelete_Click(object sender, RoutedEventArgs e)
        {
            if (ListItems.SelectedItem != null)
            {
                UnitsView UnitsToDelete = (UnitsView)ListItems.SelectedItem;
                string Question = string.Format("Està segur que vol esborrar la Unitat '{0}' ?", UnitsToDelete.Code);
                if (MsgManager.ShowQuestion(Question) == MessageBoxResult.Yes)
                {
                    string ErrMsg;
                    try
                    {
                        if (GlobalViewModel.Instance.HispaniaViewModel.LockRegister(UnitsToDelete, out ErrMsg))
                        {
                            GlobalViewModel.Instance.HispaniaViewModel.DeleteUnit(DataList[DataList.IndexOf(UnitsToDelete)]);
                            GlobalViewModel.Instance.HispaniaViewModel.UnlockRegister(UnitsToDelete, out ErrMsg);
                            DataChangedManagerActive = false;
                    ...
                    {
                        ErrMsg = string.Format("Error, al esborrar esborrar la Unitat '{0}'.\r\nDetalls: {1}", UnitsToDelete.Code, MsgManager.ExcepMsg(ex));
                    }
        ...
        /// <param name="sender">Object that sends the event.</param>
        /// <param name="e">Parameters with the event was sended.</param>
        private void UnitsDataControl_evAccept(UnitsView NewOrEditedUnits)
        {
            try
            {
                UnitsView NewUnit = new UnitsView(NewOrEditedUnits);
                switch (UnitsDataControl.CtrlOperation)
                {
                    case Operation.Add:
                         ValidateUnitInList(NewUnit);
                         GlobalViewModel.Instance.HispaniaViewModel.CreateUnit(NewUnit);
                         DataList = GlobalViewModel.Instance.HispaniaViewModel.Units;
                         DataChangedManagerActive = true;
                         ListItems.SelectedItem = NewUnit;
                         ListItems.UpdateLayout();
                         gbEditOrCreateItem.SetResourceReference(Control.StyleProperty, "NonEditableGroupBox");
                         HideItemPannel();
                         break;
                    case Operation.Edit:
                         ValidateUnitInList(NewUnit);
                         GlobalViewModel.Instance.HispaniaViewModel.UpdateUnit(NewUnit);
                         if (!GlobalViewModel.Instance.HispaniaViewModel.UnlockRegister(UnitsDataControl.UnitData, out string ErrMsg))
                         {
            ...
            catch (Exception ex)
            {
                ...
                MsgManager.ShowMessage(string.Format("Error, al guardar les dades de la Unitat{0}\r\nDetalls: {1}", OperationInfo, MsgManager.ExcepMsg(ex)));
            }
        }
        ...
        /// <param name="sender">Object that sends the event.</param>
        /// <param name="e">Parameters with the event was sended.</param>
        private void UnitsDataControl_evCancel()
        {
            switch (UnitsDataControl.CtrlOperation)
            {
                case Operation.Add:
        ...
        /// <param name="e"></param>
        private void Units_Closed(object sender, EventArgs e)
        {
            if (UnitsDataControl.CtrlOperation == Operation.Edit)
            {
                if (!GlobalViewModel.Instance.HispaniaViewModel.UnlockRegister(UnitsDataControl.UnitData, out string ErrMsg))
                {
                    MsgManager.ShowMessage(ErrMsg);
        ...
        /// </summary>
        /// <param name="NewUnit">Item to validate.</param>
        private void ValidateUnitInList(UnitsView NewUnit)
        {
            switch (UnitsDataControl.CtrlOperation)
            {
                case Operation.Add:
                     foreach (UnitsView Unit in DataList)
                     {
                         if (Unit.Code == NewUnit.Code)
                         {
                             throw new Exception(
                                           Manager_IU.GetText("ErrorUnits_01", 
                                                              string.Format("La Unitat '{0}' ja està definida.", Unit.Code)));
                         }
                     }
                     break;
                case Operation.Edit:
                     foreach (UnitsView Unit in DataList)
                     {
                         if ((Unit.Code == NewUnit.Code) && (Unit.Unit_Id != NewUnit.Unit_Id))
                         {
                             throw new Exception(
                                           Manager_IU.GetText("ErrorUnits_01", 
                                                              string.Format("La Unitat '{0}' ja està definida.", Unit.Code)));
        ...
        private void ActualizeUnitsFromDb()
        {
            //  Deactivate managers
                DataChangedManagerActive = false;
        ...
        private void ActualizeUnitFromDb()
        {
            //  Deactivate managers
                DataChangedManagerActive = false;
            //  Actualize Item Information From DataBase
                UnitsView SelectedItem = (UnitsView)ListItems.SelectedItem;
                UnitsView ItemInDb = GlobalViewModel.Instance.HispaniaViewModel.GetUnitFromDb(SelectedItem);
                int Index = ListItems.SelectedIndex;
                ListItems.UnselectAll();
                DataList.Remove(SelectedItem);
                DataList.Insert(Index, ItemInDb);
                ListItems.SelectedItem = ItemInDb;
                UnitsDataControl.UnitData = ItemInDb;
                ListItems.UpdateLayout();
            //  Deactivate managers
                DataChangedManagerActive = true;
        }
