...
{
    /// <summary>
    /// Interaction logic for RemoveWarehouseMovements.xaml
    /// </summary>
    public partial class RemoveWarehouseMovements : Window
    {
        #region Constants
        ...
        public RemoveWarehouseMovements(ApplicationType AppType)
        {
            InitializeComponent();
            Initialize(AppType);
        }
        ...
        private void BtnProceed_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (MsgManager.ShowQuestion("Esta segur que vol procedir a esborrar els moviments de magatzem ?") == MessageBoxResult.Yes)
                {
                    if (!GlobalViewModel.Instance.HispaniaViewModel.RemoveWarehouseMovements(out string ErrMsg))
                    {
                        MsgManager.ShowMessage(string.Format("Error, a l'esborrar els moviments de magatzem.\r\nDetalls: {0}\r\n.",
                                                             ErrMsg));
                    }
                    else
                    {
