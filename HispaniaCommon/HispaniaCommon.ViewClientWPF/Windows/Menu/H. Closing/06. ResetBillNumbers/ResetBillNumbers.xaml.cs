...
namespace HispaniaCommon.ViewClientWPF.Windows
{
    ...
    /// </summary>
    public partial class ResetBillNumbers : Window
    {
        #region Constants
        ...
        public ResetBillNumbers(ApplicationType AppType)
        {
            InitializeComponent();
            Initialize(AppType);
        }
        ...
        #region Proceed with Reset of Bill Number
        ...
        private void BtnProceed_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (MsgManager.ShowQuestion("Esta segur que vol procedir a resetejar el numero de factura ?") == MessageBoxResult.Yes)
                {
                    GlobalViewModel.Instance.HispaniaViewModel.ResetIdentityValueTable(new BillsView());
                }
                else MsgManager.ShowMessage("Operació cancel·lada per l'usuari.", MsgType.Information);
            }
            catch (Exception ex)
            {
                MsgManager.ShowMessage(string.Format("Error, al resetejar el numero de factura.\r\nDetalls: {0}\r\n.",
