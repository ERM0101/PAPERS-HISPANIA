...
namespace HispaniaCommon.ViewClientWPF.Windows
{
    ...
    /// </summary>
    public partial class HistoricAcumCustomers : Window
    {
        #region Constants
        ...
        public HistoricAcumCustomers(ApplicationType AppType)
        {
            InitializeComponent();
            Initialize(AppType);
        }
            ...
            try
            {
                if (GlobalViewModel.IsYear(Year_Selected.ToString(), "ANY PER ACUMULAR", out string ErrMsg))
                {
                    if (MsgManager.ShowQuestion(
                             string.Format("Esta segur que vol procedir a actualizat l'acumulat dels clients per l'any {0} ?", 
                                           Year_Selected)) == MessageBoxResult.Yes)
                    {
                        if (!GlobalViewModel.Instance.HispaniaViewModel.HistoricAcumCustomers(Year_Selected, out ErrMsg))
                        {
                            MsgManager.ShowMessage(string.Format("Error, a l'actualitzar els acumulats de client.\r\nDetalls: {0}\r\n.",
                                                                 ErrMsg));
                        }
                        else
                        {
