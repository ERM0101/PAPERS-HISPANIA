...
namespace HispaniaCommon.ViewClientWPF.Windows
{
    ...
    /// </summary>
    public partial class DiaryBandagesList : Window
    {
        #region Constants
        ...
        public DiaryBandagesList(ApplicationType AppType)
        {
            InitializeComponent();
            Initialize(AppType);
        }
        ...
        private void BtnPrint_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                int Bill_Id_From = ((BillsView)cbBill_Id_From.SelectedValue).Bill_Id;
                int Bill_Id_Until = ((BillsView)cbBill_Id_Until.SelectedValue).Bill_Id;
                decimal YearQuery = ckbYearToFilter.IsChecked == true ? DateTime.Now.Year - 1 : DateTime.Now.Year;
                try
                {
                    GlobalViewModel.Instance.HispaniaViewModel.RefreshDiaryBandages(Bill_Id_From.ToString(), Bill_Id_Until.ToString(), YearQuery);
                    DiaryBandagesReportView.CreateReport(Bill_Id_From.ToString(), Bill_Id_Until.ToString(), 
                                                         GlobalViewModel.Instance.HispaniaViewModel.DiaryBandagesDict);
                    if (MsgManager.ShowQuestion("Vol marcar les factures de per les que s'ha generat l'informe ?") == MessageBoxResult.Yes)
                    {
                        GlobalViewModel.Instance.HispaniaViewModel.UpdateMarkedFlagCustomerOrder(Bill_Id_From, Bill_Id_Until, 
                                                                                                 (DateTime)dtpDiaryData.SelectedDate, YearQuery);
                    }
                    Close();
                }
                catch (Exception ex)
        ...
        private void UpdateGUI(bool IsCurrentYear)
        {
            //  Update or Initialize Controls.
                ...
                tbDiaryData.Text = GlobalViewModel.GetStringFromDateTimeValue(dtpDiaryData.SelectedDate);
                tbLast_Bill.Text = LastBill.ToString();
                DataChangedManagerActive = true;
        }
