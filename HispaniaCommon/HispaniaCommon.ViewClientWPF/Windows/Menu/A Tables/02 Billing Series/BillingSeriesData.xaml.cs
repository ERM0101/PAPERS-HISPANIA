...
namespace HispaniaCommon.ViewClientWPF.UserControls
{
    ...
    /// </summary>
    public partial class BillingSeriesData : UserControl
    {
        #region Constants
        ...
        /// Billing Serie is correct.
        /// </summary>
        /// <param name="NewOrEditedBillingSeries">New or Edited Serie.</param>
        public delegate void dlgAccept(BillingSeriesView NewOrEditedBillingSeries);
        ...
        /// <summary>
        /// Event produced when the Button Accept is pressed and the data of the Billing Serie is correct.
        /// </summary>
        ...
        /// <summary>
        /// Store the Billing Serie data to manage.
        /// </summary>
        private BillingSeriesView m_BillingSerie = null;
        ...
        public BillingSeriesView BillingSerie
        {
            ...
            set
            {
                if (value != null)
                {
                    ...
                    m_BillingSerie = value;
                    EditedBillingSerie = new BillingSeriesView(m_BillingSerie);
                    LoadDataInControls(m_BillingSerie);
                }
                else throw new ArgumentNullException("Error, no s'han trobat les dades de la Serie a carregar."); 
            }
        }
        ...
        public Operation CtrlOperation
        {
            ...
            set
            {
                ...
                {
                    case Operation.Show :
                         if (BillingSerie == null) throw new InvalidOperationException("Error, impossible visualitzar un Serie sense dades.");
                         tbCancel.Text = "Tornar";
                         break;
                    case Operation.Add:
                         BillingSerie = new BillingSeriesView();
                         tbCancel.Text = "Cancel·lar";
                         break;
                    case Operation.Edit:
                         if (BillingSerie == null) throw new InvalidOperationException("Error, impossible editar una Serie sense dades.");
                         tbCancel.Text = "Cancel·lar";
                         break;
                }
        ...
        /// <summary>
        /// Get or Set the Edited Billing serie information.
        /// </summary>
        private BillingSeriesView EditedBillingSerie
        {
            get;
            set;
        }
        ...
        /// </summary>
        private void LoadDataInControls(BillingSeriesView Data)
        {
            //  Deactivate managers
                DataChangedManagerActive = false;
            //  Load Data in the Window Controls.
                tbSerieName.Text = Data.Name;
                tbSerieAlias.Text = Data.Alias;
        ...
        private void LoadManagers()
        {
            //  Define controls managers
                DataChangedManagerActive = true;
            //  TextBox
                tbSerieName.PreviewTextInput += TBPreviewTextInput;
                tbSerieName.TextChanged += TBDataChanged;
                tbSerieAlias.PreviewTextInput += TBPreviewTextInput;
                tbSerieAlias.TextChanged += TBDataChanged;
            //  Buttons
        ...
        {
            if (sender == tbSerieName) e.Handled = ! GlobalViewModel.IsValidBillingSerieNameChar(e.Text);
            else if (sender == tbSerieAlias) e.Handled = ! GlobalViewModel.IsValidDescriptionChar(e.Text);
        }
        ...
        private void TBDataChanged(object sender, TextChangedEventArgs e)
        {
            if (DataChangedManagerActive)
            {
                ...
                try
                {
                    if (sender == tbSerieName) EditedBillingSerie.Name = value;
                    else if (sender == tbSerieAlias) EditedBillingSerie.Alias = value;
                }
                catch (Exception ex)
                {
                    MsgManager.ShowMessage(MsgManager.ExcepMsg(ex));
                    LoadDataInControls(EditedBillingSerie);
                }
                AreDataChanged = (EditedBillingSerie != BillingSerie);
                DataChangedManagerActive = true;
            }
        }
        ...
        {
            BillingSeriesAttributes ErrorField = BillingSeriesAttributes.None;
            try
            {
                if ((CtrlOperation == Operation.Add) || (CtrlOperation == Operation.Edit))
                {
                    EditedBillingSerie.Validate(out ErrorField);
                    EvAccept?.Invoke(new BillingSeriesView(EditedBillingSerie));
                }
            }
            catch (Exception ex)
            {
        ...
        {
            EditedBillingSerie.RestoreSourceValues(BillingSerie);
            LoadDataInControls(EditedBillingSerie);
            AreDataChanged = (EditedBillingSerie != BillingSerie);
        }
        ...
        /// </summary>
        /// <param name="ErrorField"></param>
        public void RestoreSourceValue(BillingSeriesAttributes ErrorField)
        {
            EditedBillingSerie.RestoreSourceValue(BillingSerie, ErrorField);
            LoadDataInControls(EditedBillingSerie);
            AreDataChanged = (EditedBillingSerie != BillingSerie);
        }
