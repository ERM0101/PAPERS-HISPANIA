...
namespace HispaniaCommon.ViewClientWPF.Windows
{
    ...
    /// </summary>
    public partial class CustomerOrderMovementsData : Window
    {
        #region Constants
        ...
        /// </summary>
        private CustomerOrderMovementsView m_Data= null;
        ...
        /// Store the data to show in List of Items.
        /// </summary>
        public CustomerOrderMovementsView Data
        {
            get
            {
                return (m_Data);
            ...
            set
            {
                if (value != null)
                {
                    ...
                    m_Data = value;
                    EditedCustomerOrderMovement = new CustomerOrderMovementsView(m_Data);
                    if (m_Data.Good != null)
                    {
        ...
        /// <summary>
        /// Get or Set the Edited Customer Order Movement information.
        /// </summary>
        public CustomerOrderMovementsView EditedCustomerOrderMovement
        {
            get;
            set;
        }
        ...
        public CustomerOrderMovementsData(ApplicationType AppType)
        {
            InitializeComponent();
        ...
        private void InitNonEditableControls()
        {
            NonEditableControls = new List<Control>
            {
                ...
                lblValue,
                tbValue,
                lblCustomerOrderMovementId,
                tbCustomerOrderMovementId,
        ...
        /// </summary>
        private void LoadDataInControls(CustomerOrderMovementsView movement)
        {
            //  Deactivate managers
                DataChangedManagerActive = false;
            //  CustomerOrder Controls
                tbCustomerOrderId.Text = GlobalViewModel.GetStringFromIntIdValue(movement.CustomerOrder.CustomerOrder_Id);
                tbCustomerOrderMovementId.Text = movement.CustomerOrderMovement_Id_Str;
        ...
        private void TBDataChanged(object sender, TextChangedEventArgs e)
        {
            if (DataChangedManagerActive)
            {
                ...
                try
                {
                    if (sender == tbRemark) EditedCustomerOrderMovement.Remark = value;
                    else if (sender == tbInternalRemark) EditedCustomerOrderMovement.Internal_Remark = value;
                    else if (sender == tbGoodDescription) EditedCustomerOrderMovement.Description = value;
                }
                catch (Exception ex)
                {
                    MsgManager.ShowMessage(MsgManager.ExcepMsg(ex));
                    LoadDataInControls(EditedCustomerOrderMovement);
                }
                AreDataChanged = (EditedCustomerOrderMovement != Data);
                DataChangedManagerActive = true;
            }
        }
        ...
        private void TBUnitDataChanged(object sender, TextChangedEventArgs e)
        {
            if (DataChangedManagerActive)
            {
                ...
                try
                {
                    ...
                    if (sender == tbUnitBilling) EditedCustomerOrderMovement.Unit_Billing = value;
                    else if (sender == tbUnitShipping)
                    {
                        EditedCustomerOrderMovement.Unit_Shipping = value;
                        if ((EditedCustomerOrderMovement.Good != null) &&
                            (EditedCustomerOrderMovement.Good.Conversion_Factor != 0))
                        {
                            decimal NewUnitBilling = EditedCustomerOrderMovement.Unit_Shipping *
                                                     EditedCustomerOrderMovement.Good.Conversion_Factor;
                            EditedCustomerOrderMovement.Unit_Billing = NewUnitBilling;
        ...
        private void TBCurrencyDataChanged(object sender, TextChangedEventArgs e)
        {
            if (DataChangedManagerActive)
            {
                ...
                try
                {
                    ...
                    decimal value = GlobalViewModel.GetUIDecimalValue(tbInput.Text);
                    if (sender == tbRetailPrice) EditedCustomerOrderMovement.RetailPrice = value;
                    else if (sender == tbComission) EditedCustomerOrderMovement.Comission = value;
                }
                catch (Exception ex)
                {
                    MsgManager.ShowMessage(MsgManager.ExcepMsg(ex));
        ...
        private void TBPercentDataChanged(object sender, TextChangedEventArgs e)
        {
            if (DataChangedManagerActive)
            {
                ...
                try
                {
                    ...
                    decimal value = GlobalViewModel.GetUIDecimalValue(tbInput.Text);
                    if (sender == tbComission) EditedCustomerOrderMovement.Comission = value;
                }
                catch (Exception ex)
                {
                    MsgManager.ShowMessage(MsgManager.ExcepMsg(ex));
        ...
        private void ChkbAccording_Unchecked(object sender, RoutedEventArgs e)
        {
            ...
            {
                DataChangedManagerActive = false;
                EditedCustomerOrderMovement.According = false;
                AreDataChanged = (EditedCustomerOrderMovement != Data);
                DataChangedManagerActive = true;
            }
        }
        ...
        private void ChkbAccording_Checked(object sender, RoutedEventArgs e)
        {
            ...
            {
                DataChangedManagerActive = false;
                EditedCustomerOrderMovement.According = true;
                AreDataChanged = (EditedCustomerOrderMovement != Data);
                DataChangedManagerActive = true;
            }
        }
        ...
        {
            CustomerOrderMovementsAttributes ErrorField = CustomerOrderMovementsAttributes.None;
            try
            {
                EditedCustomerOrderMovement.Validate(out ErrorField);
                IsCanceled = false;
                Close();
            }
            catch (FormatException fex)
            {
                ...
                EditedCustomerOrderMovement.RestoreSourceValue(Data, ErrorField);
                LoadDataInControls(EditedCustomerOrderMovement);
                AreDataChanged = (EditedCustomerOrderMovement != Data);
            }
            catch (Exception ex)
        ...
        private void BtnHistoric_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                CustomersView Customer = EditedCustomerOrderMovement.CustomerOrder.Customer;
                HistoCustomersWindow = new HistoCustomers(AppType, HistoCustomersMode.CustomerOrderMovementLoad)
                {
                    DataList = GlobalViewModel.Instance.HispaniaViewModel.GetHistoCustomers(Customer.Customer_Id, true),
                    Data = Customer
                };
        ...
        private void HistoCustomersWindow_Closed(object sender, EventArgs e)
        {
            //  Actualize List of Customer Order Movements from historic of movements.
                ...
                {
                    EditedCustomerOrderMovement = new CustomerOrderMovementsView()
                    {
                        CustomerOrderMovement_Id = Data.CustomerOrderMovement_Id,
                    ...
                    string GoodKey = GlobalViewModel.Instance.HispaniaViewModel.GetKeyGoodView(histoCustomer.Good);
                    if (!((SortedDictionary<string, GoodsView>)cbGood.ItemsSource).ContainsKey(GoodKey))
                    {
                        ((SortedDictionary<string, GoodsView>)cbGood.ItemsSource).Add(GoodKey, new GoodsView(histoCustomer.Good));
                    }
                }
                LoadDataInControls(EditedCustomerOrderMovement);
                AreDataChanged = (Data != EditedCustomerOrderMovement);
        ...
        private void BtnAcceptSearch_Click(object sender, RoutedEventArgs e)
        {
            if (DataChangedManagerActive)
            {
                ...
                if (cbGood.SelectedItem != null)
                {
                    ...
                    if ((EditedCustomerOrderMovement.Good is null) || (EditedCustomerOrderMovement.Good.Good_Id != goodSelected.Good_Id))
                    {
                        EditedCustomerOrderMovement.Good = goodSelected;
                        EditedCustomerOrderMovement.Description = goodSelected.Good_Description;
                        EditedCustomerOrderMovement.Unit_Shipping_Definition = goodSelected.Good_Unit.Shipping;
                        EditedCustomerOrderMovement.Unit_Billing_Definition = goodSelected.Good_Unit.Billing;
                        if (EditedCustomerOrderMovement.Good.Conversion_Factor != 0)
                        {
                            decimal NewUnitBilling = EditedCustomerOrderMovement.Unit_Shipping *
                                                     EditedCustomerOrderMovement.Good.Conversion_Factor;
                            EditedCustomerOrderMovement.Unit_Billing = NewUnitBilling;
                        ...
                        //UpdateUnitsDataAndFieldsState(EditedCustomerOrderMovement);
                        EditedCustomerOrderMovement.Unit_Billing = 0;
                        EditedCustomerOrderMovement.Unit_Shipping = 0;
                        EditedCustomerOrderMovement.RetailPrice = 0;
                        EditedCustomerOrderMovement.Comission = 0;
                        EditedCustomerOrderMovement.According = false;
                        EditedCustomerOrderMovement.Remark = string.Empty;
                        EditedCustomerOrderMovement.Internal_Remark = string.Empty;
                        AreDataChanged = (EditedCustomerOrderMovement != Data);
                        LoadDataInControls(EditedCustomerOrderMovement);
        ...
        private void UpdateUnitsDataAndFieldsState(CustomerOrderMovementsView movement)
        {
