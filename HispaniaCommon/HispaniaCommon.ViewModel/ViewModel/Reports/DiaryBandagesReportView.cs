...
namespace HispaniaCommon.ViewModel
{
    ...
    /// </summary>
    public static class DiaryBandagesReportView
    {
        #region Attributes
        ...
        public static void CreateReport(string Bill_Id_From, string Bill_Id_Until,
                                        SortedDictionary<int, DiaryBandagesView> diaryBandages)
        {
            Document doc = null;
            ...
            try
            {
                ...
                writer = ReportView.GetPDF_PdfWriter(doc, PDF_Report_Types.Listing, "DiariVendes", out string PDF_FileName);
                doc.AddTitle("Diari de Vendes");
                doc.AddCreator("Hispania Papers S.L.");
                doc.Open();
                List<Tuple<string, int, string>> Details = new List<Tuple<string, int, string>>(1)
                {
                    new Tuple<string, int, string>("Numero de registres : ", 4, diaryBandages.Count.ToString())
                };
                string Title = string.Format("DIARI DE VENDES ENTRE LA FACTURA '{0}' I LA FACTURA '{1}'",
                                             Bill_Id_From, Bill_Id_Until);
                CommonReportView.InsertTitle(doc, Title, Details, 55);
                AddDiaryBandageInfo(doc, diaryBandages);
        ...
        private static void AddDiaryBandageInfo(Document doc, SortedDictionary<int, DiaryBandagesView> diaryBandages)
        {
            ...
            List<List<Tuple<string, PDF_Align>>> Items = new List<List<Tuple<string, PDF_Align>>>(diaryBandages.Count);
            ...
            foreach (DiaryBandagesView diaryBandage in diaryBandages.Values)
            {
                string Bill_Id = GlobalViewModel.GetStringFromIntIdValue(diaryBandage.Bill_Id);
                string Bill_Date = GlobalViewModel.GetStringFromDateTimeValue(diaryBandage.Bill_Date);
                string Customer_Id = GlobalViewModel.GetStringFromIntIdValue(diaryBandage.Customer_Id);
                string Company_Name = diaryBandage.Company_Name;
                decimal GrossAmount = diaryBandage.GrossAmount;
                decimal EarlyPayementDiscountAmount = diaryBandage.EarlyPayementDiscount;
                decimal TaxableBaseAmount = diaryBandage.TaxableBaseAmount;
                decimal IVAAmount = diaryBandage.IVAAmount;
                decimal SurchargeAmount = diaryBandage.SurchargeAmount;
                decimal TotalAmount = diaryBandage.Total;
